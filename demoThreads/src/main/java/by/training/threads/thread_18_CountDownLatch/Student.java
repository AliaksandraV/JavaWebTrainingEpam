package by.training.threads.thread_18_CountDownLatch;
...
// отправка ответа
        ...
        try {
            countDown.await(); // ожидание проверки заданий } catch (InterruptedException e) {
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
// подсчет средней оценки за все задачи
        float averageMark = 0;
        for (Task inWork : taskList) {
// выполнение задания
